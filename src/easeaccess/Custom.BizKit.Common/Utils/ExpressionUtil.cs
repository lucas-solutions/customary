...
namespace Custom.Utils
{
    ...
    internal static class ExpressionUtil
    {
        ...
        public static Func<TArg, TResult> CreateExpression<TArg, TResult>(
            Func<Expression, UnaryExpression> body)
        {
            ParameterExpression inp = Expression.Parameter(typeof(TArg), "inp");
            try
            {
                return Expression.Lambda<Func<TArg, TResult>>(body(inp), inp).Compile();
            }
            catch (Exception ex)
            {
                ...
                catch (InvalidOperationException)
                {
                            ...
                            typeof(TArg2) == typeof(TResult)))
                    {
                        ...
                        Expression castLhs = typeof(TArg1) == typeof(TResult) ?
                                lhs : (Expression)Expression.Convert(lhs, typeof(TResult));
                        Expression castRhs = typeof(TArg2) == typeof(TResult) ?
                                rhs : (Expression)Expression.Convert(rhs, typeof(TResult));
