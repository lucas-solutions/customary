...
namespace Custom.ComponentModel
{
    ...
    public sealed class TypeDescriptor
    {
        ...
        private static string GetExtenderCollisionSuffix(MemberDescriptor member)
        {
            ...
            {
                name = (Interlocked.Increment(ref _collisionIndex) - 1).ToString(CultureInfo.InvariantCulture);
            }
        ...
        {
            RefreshEventHandler handler = Volatile.Read<RefreshEventHandler>(ref Refreshed);
            if (handler != null)
            {
