Ext.define('Ext.screen.Panel', function () {
    ...
    function init(mt) {
        ...
        that.arrange = ScreenArranges[mt.arrange || "tabs"](that.form, that.grids);

        return that;
    };

    function afterClose(panel, e) {
        panel.fireEvent('afterClose');
    };

    function beforeClose(panel, e) {
        if (this.form.getForm().isDirty()) {
            Ext.Msg.confirm('Changes', 'Changes are not saved. Continue?', function (answer) {
                if (answer === 'yes') {
                    //panel.hide(null); // null to "unset" animation target
                    //panel.destroy();
                    //this.onClose(window);
                    panel.fireEvent('afterClose');
                }
    ...
    return {
        ...
        initComponent: function () {
            ...
            delete this.initialConfig.metadata;
            ...
            // install events
            this.addEvents('afterClose', 'isDirty');

            Ext.apply(this, that.arrange, this.initialConfig);
